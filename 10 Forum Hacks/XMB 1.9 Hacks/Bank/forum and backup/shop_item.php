...
/// Payed Access
	else if($item[feature] == "payforacces1" || $item[feature] == "payforacces2" || $item[feature] == "payforacces3" || $item[feature] == "payforacces4" || $item[feature] == "payforacces5" || $item[feature] == "payforacces6" || $item[feature] == "payforacces7" || $item[feature] == "payforacces8") {
		if($item[feature] == "payforacces1"){
			...
			$fiddl = "17";
		}elseif($item[feature] == "payforacces4"){
			$fiddl = "20";
		}elseif($item[feature] == "payforacces5"){
			$fiddl = "21";
		}elseif($item[feature] == "payforacces6"){
			$fiddl = "55";
		}elseif($item[feature] == "payforacces7"){
			$fiddl = "22";
		}elseif($item[feature] == "payforacces8"){
			$fiddl = "23";
		}
		...
		if(!$yessubmit && !$nosubmit){
			...
			if($memberinfo['money'] >= $item['price'] && $item['stock'] != 0) { $fidds = explode("|#|", $fiddl); 
				...
				$outputtext = "You have been granded acces...";
			} else {
				$outputtext = "$lang_shop_item_noupdate";
			}
			eval("\$info = \"".template("shop_info")."\";");
	...
	// Items
		...
		if(!$submit) {
			...
			if($memberinfo['money'] >= $item['price'] && $item['stock'] != 0) {
				...
				insert_bank_trans('0', $item[owner], $item[price], $item[itemname], 'Goods', 'Shop Purchase');
